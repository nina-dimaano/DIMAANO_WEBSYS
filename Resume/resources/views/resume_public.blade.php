...
<div class="header-bar">
    <div class="welcome-text">Welcome, {{ Auth::user()->username }}!</div>
...
<div class="container">
    <div class="left-column">
        @if($user->profile_picture)
            <img src="{{ asset('storage/' . $user->profile_picture) }}" alt="Profile Photo" class="profile-photo">
        @else
            <img src="https://via.placeholder.com/150" alt="Profile Photo" class="profile-photo">
        @endif
        <div class="name">{{ $user->fullname ?? 'Your Name' }}</div>
        <div class="job-title">{{ $user->title ?? 'Your Title' }}</div>

        <div class="section-title">✉️ Contact</div>
        @if($user->address)<div class="contact-item">📍 {{ $user->address }}</div>@endif
        @if($user->website)<div class="contact-item">🌐 {{ $user->website }}</div>@endif
        @if($user->email)<div class="contact-item">📧 {{ $user->email }}</div>@endif
        @if($user->phone)<div class="contact-item">☎️ {{ $user->phone }}</div>@endif
        ...
        @if(is_array($user->skills) && count($user->skills) > 0)
            @foreach($user->skills as $skill)
                ...
                <div class="skill-item">
                    ...
                    <div class="skill-bar">
                        <div class="skill-level" style="width: {{ ($skill['level'] ?? 1) * 20 }}%"></div>
                    </div>
                </div>
                @endif
            @endforeach
    ...
    <div class="right-column">
        ...
        <p class="about-text">{{ $user->about ?? 'No information provided yet.' }}</p>

        <div class="right-section">🎓 Education</div>
        @if(is_array($user->education) && count($user->education) > 0)
            @foreach($user->education as $edu)
            <div class="education-item">
                <div class="year">{{ $edu['year'] ?? 'N/A' }}</div>
                <div class="type">{{ $edu['type'] ?? 'N/A' }}</div>
                <div class="school">{{ $edu['school'] ?? 'N/A' }}</div>
            </div>
            @endforeach
        @else
        ...
        @if(is_array($user->experience) && count($user->experience) > 0)
            @foreach($user->experience as $exp)
            <div class="exp-item">
                <div class="year">{{ $exp['year'] ?? 'N/A' }}</div>
                <div class="seminar">{{ $exp['seminar'] ?? 'N/A' }}</div>
                <div class="position">{{ $exp['position'] ?? 'N/A' }}</div>
                @if(isset($exp['tasks']) && is_array($exp['tasks']) && count($exp['tasks']) > 0)
                <ul class="task-list">
